...
================================================================================
Initializing the client certificates issuing (third-level) client-chain CA...
...
if ! test -s "$serialFile" ; then
  # echo "Client CA" | od -t x1
  echo "436c69656e74204341000000000001" > "$serialFile"
fi
...
if ! test -s cacert.pem ; then
  echo "Create a CSR for the THIRD LEVEL client CA..."
  openssl req -passin file:private/pass.phrase.txt -config "$1.req-self.conf" -new -key private/cakey.pem -out private/cacert.csr -days 3650

  # print the CSR
  #openssl req -text -noout -verify -in private/cacert.csr

  echo "Let the SECOND LEVEL CA sign the certificate..."
  # PROBLEM: to make this work, we must temporarily switch to the PARENT CA environment:
  csrPath="$( realpath private/cacert.csr )"
  pushd "$3"
  pwd
  openssl ca -passin file:private/pass.phrase.txt -config "$2" -batch -in "$csrPath" -extensions v3_ca -out "$wd/cacert.pem"
  popd
